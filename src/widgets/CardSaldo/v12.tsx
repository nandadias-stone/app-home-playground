...
import styles from './v12.module.css';

type CardSaldoV12Props = {
  nomeUsuario?: string;
  saldo?: number;
  /** Hora do dia (0-23) para forçar contexto na demonstração. Sem esta prop, usa Date.now(). */
  forcarHora?: number;
  onPix?: () => void;
  onTransferir?: () => void;
  onPagar?: () => void;
  ...
  return (
    <Card padding="none">
      <div className={styles.greeting}>
        <p className={`${styles.hello} jade-heading-medium`}>{saudacao}</p>
        {phase === 'ready' ? (
          <p className={`${styles.message} jade-heading-small ${styles.fadeIn}`}>{mensagem}</p>
        ) : phase === 'loading' ? (
      ...
      <button className={styles.saldoRow} type="button">
        <span className={`${styles.label} jade-text-medium-regular`}>Saldo da conta</span>
        <span className={styles.amountWrap}>
          <span className={`${styles.amount} jade-text-large-semibold`}>{formatBRL(saldo)}</span>
